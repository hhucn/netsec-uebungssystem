...
Grading = collections.namedtuple('Grading', ['id', 'submission_id', 'task_id', 'comment', 'time', 'decipoints', 'grader'])


def get_grade_for_task(db, task_id, submission_id):
    db.cursor.execute(
        """SELECT id, comment, time, decipoints, grader FROM grading WHERE submission_id = ? AND task_id = ?""", (submission_id, task_id))
...
def set_grade_for_task(db, task_id, submission_id, comment, time, decipoints, grader):
    ...
    if db.cursor.fetchone():
        ...
        db.cursor.execute("""UPDATE grading SET comment = ?, time = ?, decipoints = ?, grader = ? WHERE
                             submission_id = ? AND task_id = ?""", (comment, time, decipoints, grader, submission_id, task_id))
    else:
